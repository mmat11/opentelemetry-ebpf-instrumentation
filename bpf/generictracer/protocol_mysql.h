...
struct mysql_hdr {
    ...
    // Metadata
    bool skip; // Signals whether to skip or not the first 4 bytes in the current buffer
...
static inline void mysql_store_state_data(connection_info_t *conn_info, const unsigned char *data) {
    ...
    if (state_data == NULL) {
        ...
        bpf_probe_read(&new_state_data, 3, (void *)data);
        bpf_probe_read(&new_state_data.sequence_id, 1, (void *)(data + 3));
        new_state_data.sent = true;
        bpf_map_update_elem(&mysql_state, conn_info, &new_state_data, BPF_ANY);
    } else {
        bpf_probe_read(state_data, 4, (void *)data);
        state_data->sent = true;
...
static inline int mysql_parse_fixup_header(connection_info_t *conn_info,
    ...
    if (state_data != NULL) {
        ...
        bpf_probe_read(&hdr->command_id, 1, (void *)data);
        hdr->skip = true;
...
static void mysql_request_handle(struct mysql_hdr *hdr,
    ...
    e->query_len = hdr->payload_length - 1;
...
static bool is_err_packet(struct mysql_hdr *hdr, uint8_t command_id, size_t data_len) {
    uint8_t min_data_len = hdr->skip ? 5 : 9;
...
static int parse_response_packet_type(struct mysql_hdr *hdr, const uint8_t *data, size_t data_len) {
    ...
    uint8_t command_id;
    bpf_probe_read(&command_id, 1, data + mysql_command_offset(hdr));
...
static inline uint8_t is_mysql(connection_info_t *conn_info,
    ...
    case MYSQL_COM_QUERY:
        ...
        if (offset == -1) {
            bpf_dbg_printk(
                "is_mysql: COM_QUERY or COM_PREPARE found, but buf doesn't contain an sql query");
            return 0;
        }
